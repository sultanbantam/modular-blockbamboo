      ...
      <header className="absolute top-0 left-0 w-full z-50 bg-stone-900/95 border-b border-stone-800 backdrop-blur-md shadow-lg flex flex-wrap lg:flex-nowrap justify-between items-center px-4 py-2 pointer-events-auto">
        ...
        <div className="flex items-center gap-4 lg:gap-6 w-full lg:w-auto justify-between lg:justify-start mb-2 lg:mb-0">
          <div className="flex flex-col">
            <h1 className="text-xl md:text-2xl font-bold text-green-500 drop-shadow-md leading-tight">BlockBamboo</h1>
        ...
        <div className="flex gap-2 items-center overflow-x-auto custom-scrollbar w-full lg:w-auto pb-1 lg:pb-0 pt-2 lg:pt-0">
          ...
          <div className="flex gap-1 border-r border-stone-700 pr-2">
            <button 
              ...
              className={`p-2 rounded transition-colors flex items-center gap-1 ${(pastBlocks.length === 0 || isUndoRedoLocked) ? 'text-stone-600 cursor-not-allowed' : 'text-stone-300 hover:bg-stone-800 hover:text-white'}`}
              title={isUndoRedoLocked ? "Batal (Terbuka di Level 3)" : "Batal (Undo)"}
            >
            ...
            <button 
              ...
              className={`p-2 rounded transition-colors flex items-center gap-1 ${(futureBlocks.length === 0 || isUndoRedoLocked) ? 'text-stone-600 cursor-not-allowed' : 'text-stone-300 hover:bg-stone-800 hover:text-white'}`}
              title={isUndoRedoLocked ? "Ulangi (Terbuka di Level 3)" : "Ulangi (Redo)"}
            >
          ...
          <div className="flex gap-1 border-r border-stone-700 pr-2">
            <button 
              ...
              className="p-2 rounded transition-colors text-stone-300 hover:bg-stone-800 hover:text-amber-400"
              title="Simpan / Muat Desain (Save/Load)"
            >
              <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 7H5a2 2 0 00-2 2v9a2 2 0 002 2h14a2 2 0 002-2V9a2 2 0 00-2-2h-3m-1 4l-3 3m0 0l-3-3m3 3V4" /></svg>
            </button>
            <button 
              onClick={() => setShowMultiplayerLobby(true)}
              className={`p-2 rounded transition-colors flex items-center gap-1 ${roomId ? 'bg-green-900/30 text-green-400 border border-green-800/50' : 'text-stone-300 hover:bg-stone-800 hover:text-blue-400'}`}
              title="Mode Co-Op (Multiplayer)"
            >
          ...
          <div className="flex gap-1 border-r border-stone-700 pr-2">
            <button 
              ...
              className={`p-2 rounded transition-colors ${isPanMode ? 'bg-amber-600 text-white' : 'text-stone-300 hover:bg-stone-800 hover:text-white'}`}
              title={isPanMode ? "Kursor Bebas (Navigasi Kamera)" : "Mode Pembangunan (Pasang/Edit Blok)"}
            >
            ...
            <button 
              ...
              className={`p-2 rounded transition-colors ${showHelpers ? 'bg-amber-600 text-white' : 'text-stone-300 hover:bg-stone-800 hover:text-white'}`}
              title="Tampilkan / Sembunyikan Bantuan Pemasangan (Snap Points)"
            >
              {showHelpers ? 
          ...
          <div className="flex gap-1 border-r border-stone-700 pr-2">
            ...
            <button 
                ...
                className="p-2 rounded transition-colors text-stone-400 hover:bg-stone-800 hover:text-white"
                title="Reset Kamera"
              >
          ...
          <div className="flex gap-1 pl-1">
            <button 
              ...
              className="p-2 flex items-center gap-2 rounded transition-colors text-purple-400 hover:bg-stone-800"
              title="Buka Dompet & Dashboard Pi"
            >
              <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
              <span className="font-bold hidden xl:block text-sm">Dompet Pi</span>
            </button>
          </div>
        </div>
      </header>
